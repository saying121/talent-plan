...
fn test_concurrent_starts_2b() {
    ...
    'outer: for tried in 0..5 {
        ...
        for ii in 0..5 {
            ...
            idx_rxs.push(rx);
            let node = cfg.rafts.lock().unwrap()[leader]
                .clone()
                .unwrap();
            cfg.net
...
fn test_unreliable_agree_2c() {
    ...
    for iters in 1..50 {
        for j in 0..4 {
            ...
            let (tx, rx) = oneshot::channel();
            thread::spawn(move || {
                c.one(Entry { x: (100 * iters) + j }, 1, true);
                tx.send(())
                    .map_err(|e| panic!("send failed: {:?}", e))
            });
            dones.push(rx);
...
fn test_figure_8_unreliable_2c() {
    ...
    let mut random = rand::thread_rng();
    cfg.one(Entry { x: random.gen::<u64>() % 10000 }, 1, true);
    ...
    for iters in 0..1000 {
        ...
        for i in 0..servers {
            if cfg.rafts.lock().unwrap()[i]
                .as_ref()
                .unwrap()
                .start(&Entry { x: random.gen::<u64>() % 10000 })
                .is_ok()
                && cfg.connected[i]
            {
                leader = Some(i);
    ...
    cfg.one(Entry { x: random.gen::<u64>() % 10000 }, servers, true);
